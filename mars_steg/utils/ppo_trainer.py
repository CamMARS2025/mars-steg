...
class PPOTrainer(BaseTrainer):
    ...
    def step(
        ...
        """
        Run a PPO optimization step given lists of queries, responses, and reward scores.
        This complete function demonstrates improved mini‐batch processing by cleaning up
        """
        ...
        if self.config.use_score_scaling:
            ...
            score_scaling_factor = self.running.std.to(scores.dtype) + torch.finfo(scores.dtype).eps
            if self.config.use_score_norm:
                scores = (scores - self.running.mean.to(scores.dtype)) / score_scaling_factor
            else:
                scores = scores / score_scaling_factor
        if self.config.score_clip is not None:
            scores = torch.clip(scores.float(), -self.config.score_clip, self.config.score_clip).to(scores.dtype)

        # Optionally push best model to hub.
        ...
        # Include additional model inputs.
        batch_dict.update(model_inputs)
        del model_inputs
        torch.cuda.empty_cache()

        # Optimization loop: iterate through PPO epochs and mini-batches within each backward batch.
        all_stats = []
        early_stop = False
        for epoch in range(self.config.ppo_epochs):
            ...
            # Early stopping check based on the current policy KL.
            if self.config.early_stopping:
                policykl = train_stats.get("policy/policykl", torch.tensor(0.0))
        ...
        train_stats["policy/advantages"] = torch.nan_to_num(train_stats["policy/advantages"], nan=0.0).to("cpu")
    ...
    def batched_forward_pass(
        ...
        for i in range(math.ceil(bs / fbs)):
            ...
            logprobs = selective_log_softmax(logits[:, :-1, :], input_ids[:, 1:])
            if return_logits:
